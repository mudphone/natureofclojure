...
(defn apply-behaviors [all vehicle]
  (let [sep-factor 2.0
        seek-factor 1.0        
        sep-force (fvec/*
                   (beh/separate SEPARATION-DIST all vehicle)
                   sep-factor)
        target (fvec/fvec (q/mouse-x) (q/mouse-y)) 
        seek-force (fvec/*
                    (beh/seek target vehicle)
                    seek-factor)]
    (-> vehicle
        (beh/apply-force sep-force)
        (beh/apply-force seek-force))))

(defn update-vehicles [vehicles]
  (doall (map #(->> %
                    (apply-behaviors vehicles)
                    (beh/move-vehicle)
...
(q/defsketch quil-workflow
  :title "Flocking: Separation & Seek"
  :size [SIZE-W SIZE-H]
  :setup setup
  :update update
  :draw draw
